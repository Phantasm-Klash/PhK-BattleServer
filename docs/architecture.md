...
| `protocol` | Holds battle packet headers and dispatcher guards until generated protobuf bindings are wired. It rejects client-authored result packets and now requires development key id/nonce shape on encrypted-path packet types before accepting seq state. |
...
- Encrypted-path battle packets must carry a key id and nonce-shaped packet header in the dispatcher scaffold before seq replay state advances.
