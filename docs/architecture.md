...
| `protocol` | Holds battle packet headers, encrypted packet adapter structs, and dispatcher guards until generated protobuf bindings are wired. It rejects client-authored result packets and now requires development key id/nonce/ciphertext/auth-tag shape plus per-session nonce reuse rejection on encrypted-path packet types before accepting seq state. |
