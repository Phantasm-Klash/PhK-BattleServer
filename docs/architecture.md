...
- Development signed-result callback material is generated only from the server-owned replay summary: allocated match id, frozen mode id/ruleset, player ids, replay-derived result hash, replay id, event cursor, final tick, missing-input fallback counters, accepted mode-action count, deterministic input/event trace counts, input stream hash, event stream hash, final state hash, projection-only reward JSON, and battle server key id.
...
- Battle results are bound to the allocated match id, frozen mode id, frozen ruleset version, player ids, local replay-derived result hash, replay id, replay event cursor, replay input/event/final-state hashes, settled time, and battle server key id before the business server can settle rewards.
