...
- Server-facade encrypted packet dispatch additionally requires the packet's `match_id`/`player_id` to resolve to a registered battle session and the packet `key_id` to match that session. Unknown sessions or mismatched key ids are rejected before dispatcher seq/nonce state changes.
