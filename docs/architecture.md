...
| `server` | Composes ticket verification, capacity-checked session creation, registered-ticket handshake acceptance, registered-session encrypted packet dispatch, match simulation input/tick/snapshot/reconnect calls, and idempotent battle result submission bound to the local replay summary. |
