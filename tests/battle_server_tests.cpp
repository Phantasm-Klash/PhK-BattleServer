...
bool TestBattleResultSubmission() {
    ...
    auto wrong_input_buffer_peak = valid_result;
    wrong_input_buffer_peak.result.mode_result_json = ReplaceFirst(
        valid_result.result.mode_result_json,
        "\"input_buffer_peak_record_count\":0",
        "\"input_buffer_peak_record_count\":999"
    );
    const auto wrong_input_buffer_peak_result = server.SubmitBattleResult(wrong_input_buffer_peak);
    CHECK_TRUE(!wrong_input_buffer_peak_result.ok);
    CHECK_EQ(
        wrong_input_buffer_peak_result.reason,
        std::string("input_buffer_peak_record_count_mismatch")
    );

    auto wrong_mode_action_buffer_peak = valid_result;
    wrong_mode_action_buffer_peak.result.mode_result_json = ReplaceFirst(
        valid_result.result.mode_result_json,
        "\"mode_action_buffer_peak_record_count\":1",
        "\"mode_action_buffer_peak_record_count\":999"
    );
    const auto wrong_mode_action_buffer_peak_result = server.SubmitBattleResult(wrong_mode_action_buffer_peak);
    CHECK_TRUE(!wrong_mode_action_buffer_peak_result.ok);
    CHECK_EQ(
        wrong_mode_action_buffer_peak_result.reason,
        std::string("mode_action_buffer_peak_record_count_mismatch")
    );
...
bool TestBuildSignedBattleResultCallback() {
    ...
    CHECK_EQ(
        built.signed_result.signature_hex,
        std::string(
            "5ec1fcb6b3496db93fc735ada85a23989cb78ac8c92801fb7dbcc3bfbe38b7d"
            "adaad18dadf06963dbbb251d1d4174c1c18a2a6ecf4e52a7ff9a7dfe3e9f5e05e"
        )
    );
    ...
    CHECK_EQ(
        ...
        std::string(
            ...
            "\"mode_action_count\":0,\"pending_input_tick_count\":0,\"pending_input_record_count\":0,"
            "\"input_buffer_peak_tick_count\":1,\"input_buffer_peak_record_count\":2,"
            "\"pending_mode_action_tick_count\":0,\"pending_mode_action_record_count\":0,"
            "\"mode_action_buffer_peak_tick_count\":0,\"mode_action_buffer_peak_record_count\":0,"
            "\"input_trace_count\":2,\"event_trace_count\":0,"
            "\"input_stream_hash\":\"fnv64:6b09da7d62e0941e\","
            "\"event_stream_hash\":\"fnv64:14650fb0739d0383\","
            "\"final_state_hash\":\"fnv64:72a3385f1a7c7fe3\","
            "\"replay_summary_hash\":\"sha256:dev-fnv64-94475f8b5b3d0865\","
            "\"replay_fixture_hash\":\"sha256:dev-fnv64-1c107f0eca53f2ea\","
    ...
    CHECK_TRUE(
        built.signed_result.result.mode_result_json.find("\"input_buffer_peak_record_count\":2") !=
        std::string::npos
    );
    CHECK_TRUE(
        built.signed_result.result.mode_result_json.find("\"mode_action_buffer_peak_record_count\":0") !=
        std::string::npos
    );
    CHECK_TRUE(
        built.signed_result.result.mode_result_json.find(
...
bool TestBossSimulationRejectsNinthPlayer() {
    ...
    CHECK_EQ(snapshot.mode_state.at("boss_player_p7_spawn_slot"), std::string("southwest"));
    CHECK_EQ(snapshot.mode_state.at("boss_player_p8_spawn_slot"), std::string("northwest"));
    CHECK_EQ(snapshot.players[0].x_milli, 0);
    CHECK_EQ(snapshot.players[0].y_milli, -60000);
    CHECK_EQ(snapshot.players[1].x_milli, 60000);
    CHECK_EQ(snapshot.players[1].y_milli, 0);
    CHECK_EQ(snapshot.players[2].x_milli, 0);
    CHECK_EQ(snapshot.players[2].y_milli, 60000);
    CHECK_EQ(snapshot.players[3].x_milli, -60000);
    CHECK_EQ(snapshot.players[3].y_milli, 0);
    CHECK_EQ(snapshot.players[4].x_milli, 42426);
    CHECK_EQ(snapshot.players[4].y_milli, -42426);
    CHECK_EQ(snapshot.players[5].x_milli, 42426);
    CHECK_EQ(snapshot.players[5].y_milli, 42426);
...
bool TestBossReadyToStartRequiresAllReadyPlayers() {
    ...
    CHECK_EQ(ready_snapshot.mode_state.at("boss_lifecycle_state"), std::string("combat_started"));
...
bool TestBossModeResultProjection() {
    ...
    CHECK_EQ(fixture.summary.boss_defeated, static_cast<std::uint64_t>(1));
    CHECK_EQ(fixture.summary.boss_defeated_tick, static_cast<std::uint64_t>(1));
    CHECK_EQ(fixture.replay_summary_record.boss_max_hp, static_cast<std::uint64_t>(20));
    CHECK_EQ(fixture.replay_summary_record.boss_current_hp, static_cast<std::uint64_t>(0));
    CHECK_EQ(fixture.replay_summary_record.boss_damage_total, static_cast<std::uint64_t>(20));
    CHECK_EQ(fixture.replay_summary_record.boss_defeated, static_cast<std::uint64_t>(1));
    CHECK_EQ(fixture.replay_summary_record.boss_defeated_tick, static_cast<std::uint64_t>(1));
    ...
    auto tampered_boss_defeated_summary = fixture.summary;
    tampered_boss_defeated_summary.boss_defeated = 0;
    CHECK_TRUE(ExpectedDevResultHash(tampered_boss_defeated_summary) != fixture.result_hash);
    auto tampered_boss_hp_record = fixture.replay_summary_record;
    tampered_boss_hp_record.boss_current_hp = 1;
    CHECK_TRUE(
        phk::battle::DevReplayInputStreamSummaryHash(tampered_boss_hp_record) !=
        phk::battle::DevReplayInputStreamSummaryHash(fixture.replay_summary_record)
    );
    auto tampered_boss_defeated_record = fixture.replay_summary_record;
    tampered_boss_defeated_record.boss_defeated = 0;
    CHECK_TRUE(
        phk::battle::DevReplayInputStreamSummaryHash(tampered_boss_defeated_record) !=
        phk::battle::DevReplayInputStreamSummaryHash(fixture.replay_summary_record)
    );
...
bool TestBossModeResultSubmissionRequiresBossProjection() {
    ...
    CHECK_TRUE(
        built.signed_result.result.mode_result_json.find("\"boss_lifecycle_state\":\"combat_started\"") !=
        std::string::npos
    );
...
bool TestBossRosterLocksOnPendingReadyWindow() {
    phk::battle::BattleServerConfig config;
    config.now_ms = 1782489646100;
    phk::battle::BattleServer server(config);

    for (std::size_t index = 1; index <= 4; ++index) {
        CHECK_TRUE(server.RegisterTicket(MakeModeTicket(
            "ticket-pending-roster-lock-" + std::to_string(index),
            "user-pending-roster-lock-" + std::to_string(index),
            "p" + std::to_string(index),
            "world_boss",
            "00112233445566778899fc0" + std::to_string(index)
        )).ok);
    }
    ...
    for (std::size_t index = 1; index <= 4; ++index) {
        ...
        ready.tick = 1;
        ready.seq = 1;
        ready.action_id = "boss-pending-roster-lock-ready-" + std::to_string(index);
        ready.action_type = "ready";
        ready.payload_json = "{\"ready\":true}";
        CHECK_TRUE(server.AcceptModeAction(ready).ok);
    }

    const auto pending_snapshot = server.MatchSnapshot("match-001");
    CHECK_EQ(pending_snapshot.mode_state.at("boss_combat_started"), std::string("0"));
    CHECK_EQ(pending_snapshot.mode_state.at("boss_roster_locked"), std::string("0"));
    CHECK_EQ(pending_snapshot.mode_state.at("pending_mode_action_record_count"), std::string("4"));

    const auto late_join = server.RegisterTicket(MakeModeTicket(
        "ticket-pending-roster-lock-late",
        "user-pending-roster-lock-late",
        "p5",
        "world_boss",
        "00112233445566778899fc05"
    ));
    CHECK_TRUE(!late_join.ok);
    CHECK_EQ(late_join.reason, std::string("boss_roster_locked"));
    CHECK_EQ(late_join.active_sessions_before, static_cast<std::size_t>(4));
    ...
    const auto ready_snapshot = server.TickMatch("match-001");
    CHECK_EQ(ready_snapshot.mode_state.at("boss_ready_to_start"), std::string("1"));
    CHECK_EQ(ready_snapshot.mode_state.at("boss_roster_locked"), std::string("1"));
    CHECK_EQ(ready_snapshot.players.size(), static_cast<std::size_t>(4));
    return true;
}
...
bool TestAuthoritativeReplay60TickFixture() {
    ...
    CHECK_EQ(first.Snapshot().mode_state.at("pending_input_tick_count"), std::string("0"));
    CHECK_EQ(first.Snapshot().mode_state.at("pending_input_record_count"), std::string("0"));
    CHECK_EQ(first.Snapshot().mode_state.at("input_buffer_peak_tick_count"), std::string("1"));
    CHECK_EQ(first.Snapshot().mode_state.at("input_buffer_peak_record_count"), std::string("2"));
    CHECK_EQ(first.Snapshot().mode_state.at("pending_mode_action_tick_count"), std::string("0"));
    CHECK_EQ(first.Snapshot().mode_state.at("pending_mode_action_record_count"), std::string("0"));
    CHECK_EQ(first.Snapshot().mode_state.at("mode_action_buffer_peak_tick_count"), std::string("0"));
    CHECK_EQ(first.Snapshot().mode_state.at("mode_action_buffer_peak_record_count"), std::string("0"));
...
bool TestReplayFixtureBoundary() {
    ...
    CHECK_EQ(fixture.final_snapshot.mode_state.at("pending_input_tick_count"), std::string("0"));
    CHECK_EQ(fixture.final_snapshot.mode_state.at("pending_input_record_count"), std::string("0"));
    CHECK_EQ(fixture.final_snapshot.mode_state.at("input_buffer_peak_tick_count"), std::string("1"));
    CHECK_EQ(fixture.final_snapshot.mode_state.at("input_buffer_peak_record_count"), std::string("2"));
    CHECK_EQ(fixture.final_snapshot.mode_state.at("pending_mode_action_tick_count"), std::string("0"));
    CHECK_EQ(fixture.final_snapshot.mode_state.at("pending_mode_action_record_count"), std::string("0"));
    CHECK_EQ(fixture.final_snapshot.mode_state.at("mode_action_buffer_peak_tick_count"), std::string("0"));
    CHECK_EQ(fixture.final_snapshot.mode_state.at("mode_action_buffer_peak_record_count"), std::string("0"));
    ...
    CHECK_TRUE(
        ...
        "fnv64:daa6853bacb4fdd3|fnv64:8049946f03724f36|424242|60|0|0|0|0|0"
    );
    CHECK_EQ(
        phk::battle::DevReplayInputStreamSummaryHash(summary_record),
        std::string("sha256:dev-fnv64-575ffa809639f4b8")
    );
    ...
    CHECK_TRUE(canonical_fixture_payload.find("mode=input_buffer_peak_record_count=2;") != std::string::npos);
    CHECK_TRUE(canonical_fixture_payload.find("mode=pending_input_record_count=0;") != std::string::npos);
    ...
    CHECK_EQ(
        ...
        std::string("sha256:dev-fnv64-433cb92be24ade8e")
    );
    auto tampered_fixture_summary_count = fixture;
    tampered_fixture_summary_count.summary.input_count += 1;
    CHECK_TRUE(phk::battle::DevReplayFixtureHash(tampered_fixture_summary_count) != phk::battle::DevReplayFixtureHash(fixture));
    auto tampered_fixture_summary_hash = fixture;
    tampered_fixture_summary_hash.summary.input_stream_hash = "fnv64:0000000000000000";
    CHECK_TRUE(phk::battle::DevReplayFixtureHash(tampered_fixture_summary_hash) != phk::battle::DevReplayFixtureHash(fixture));
    auto tampered_fixture_summary_event_trace = fixture;
    tampered_fixture_summary_event_trace.summary.event_trace.back() += "|tampered";
    CHECK_TRUE(phk::battle::DevReplayFixtureHash(tampered_fixture_summary_event_trace) != phk::battle::DevReplayFixtureHash(fixture));
...
bool TestReplayRecordBridgeBoundary() {
    ...
    CHECK_TRUE(canonical_record.find("fnv64:14650fb0739d0383|fnv64:72a3385f1a7c7fe3|16031087345790602692|1|0|0|0|0|0") != std::string::npos);
    ...
    CHECK_EQ(
        built.replay_record_hash,
        std::string("sha256:dev-fnv64-44d0a58dc50f8cc8")
    );
...
int main() {
	const std::vector<std::pair<std::string, bool (*)()>> tests = {
        ...
        {"BossRosterLocksOnPendingReadyWindow", TestBossRosterLocksOnPendingReadyWindow},
