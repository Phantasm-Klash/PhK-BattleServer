...
| Battle result submission | Started | Development `SignedBattleResult` boundary and verifier check match/mode binding, player-id set, result hash, replay id, settled time, key id, Ed25519 signature shape, and idempotent result hash replay through the server facade. Real result signing, protobuf serialization, and service-to-service submission are pending. |
| Battle handshake | Started | Placeholder handshake selects ChaCha20-Poly1305 labels and creates deterministic dev session metadata. Real X25519/ECDHE, HKDF, transcript hash, and server signatures are pending. |
...
| Authoritative simulation | Started | Added a dependency-light C++ simulation slice behind the server facade: fixed `kBattleTickRateHz = 60`, authoritative `BattleInput` validation, per-player seq replay rejection, input tick window checks, milli-unit movement, simplified deterministic radial bullet spawning/movement, `BattleSnapshot` mirror structs, canonical FNV-1a state hash, and replay summary input/event/final hashes. Full hit/graze/card/Boss scoring is still pending. |
| Tests | Started | CTest smoke validates protocol manifest consumption, ticket verification, ticket replay rejection, session registration, handshake acceptance, battle result submission/idempotency guards, simulation determinism, server-side authoritative input/snapshot/replay summary calls, KCP placeholder echo, and dispatcher guards. The repository checker now handles Visual Studio multi-config builds with `--config Debug` and asserts the result/simulation boundary files exist. |
