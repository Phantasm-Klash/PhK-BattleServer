...
| Decoded payload handoff | Started | Server facade now exposes development `AcceptDecodedInput` and `AcceptDecodedModeAction` adapter points for the future protobuf decode path. They bind decoded local structs to the already-validated encrypted packet header's version stamp, payload type, match id, player id, tick, and seq before simulation state or replay counters can advance. Real protobuf decode and AEAD decrypt remain pending. |
