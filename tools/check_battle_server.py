...
def main() -> int:
    ...
    if (
        ...
        or "select_round_card_mode_unsupported" not in simulation_impl
        or "select_round_card_candidate_missing" not in simulation_impl
        or "select_round_card_candidate_invalid" not in simulation_impl
    ...
    if (
        ...
        return 1
    if (
        server_impl.count("result.pending_boss_configs_before = pending_boss_config_by_match_.size()") < 4
        or server_impl.count("result.pending_boss_configs_after = pending_boss_config_by_match_.size()") < 4
    ):
    ...
    if (
        ...
        or "sha256:dev-fnv64-70b0875fedb4037c" not in tests_text
        ...
        or "sha256:dev-fnv64-82e944315b4cd1b4" not in tests_text
        or "CanonicalReplayFixturePayload(fixture)" not in tests_text
        or "sha256:dev-fnv64-e223cb92b199d91a" not in tests_text
        ...
        or "sha256:dev-fnv64-8f4392ac01c56b3f" not in tests_text
        ...
        or "sha256:dev-fnv64-9e77178bf0907b46" not in tests_text
        or "sha256:dev-fnv64-e8a3d7f2ad39b1d1" not in tests_text
